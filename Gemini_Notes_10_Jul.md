# CompTIA Network+ (N10-009) — Essential Missing Notes
**Generated by Gemini on 10 July 2026**

This document contains all the critical information missing from your current study notes, based on the audit of your `Opus_Network+_Study_V2.md` file and the official CompTIA Network+ (N10-009) exam objectives. It focuses **only** on the topics you need to learn or expand upon to pass the exam.
...
### 1. TCP In-Depth
*   **Connection Teardown:** A 4-way handshake using FIN and ACK flags. Device 1 sends FIN, Device 2 sends ACK and then its own FIN, Device 1 sends final ACK.
*   **Windowing / Flow Control:** TCP uses a "sliding window" to tell the sender how much data it can receive before needing to send an acknowledgment. This prevents overwhelming the receiver.
*   **TCP vs. UDP (Connection-oriented vs. Connectionless):** TCP establishes a reliable session (handshake), tracks data delivery, and retransmits lost packets. UDP just sends data without guaranteed delivery (best effort) but is faster.
*   **TCP Flags:**
    ...
    *   **PSH:** Push (pushes data directly to the application layer, bypassing buffers)
    *   **URG:** Urgent (tells receiver to process this data immediately)

### 2. IPv6 In-Depth
*   **Address Types:**
    *   **Global Unicast:** Like an IPv4 public address, routable on the internet (starts with `2000::/3`).
    *   **Unique Local:** Like an IPv4 private address, routable within an organization but not the internet (starts with `fc00::/7` or `fd00::/8`).
    *   **Link-Local:** Only valid on a single network link/segment. Cannot be routed. Starts with `fe80::/10`. Every IPv6 interface must have one.
    *   **Multicast:** Starts with `ff00::/8`. Replaces IPv4 broadcast.
*   **NDP (Neighbor Discovery Protocol):** Replaces ARP in IPv6. Uses ICMPv6 messages (Router Solicitation/Advertisement, Neighbor Solicitation/Advertisement) to find MAC addresses and routers on the local network.
